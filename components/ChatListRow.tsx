...
const router = useRouter()
  ...
  const row = (message?:Message) => (
    ...
    className='flex p-5 items-center space-x-2 cursor-pointer hover:bg-primary/10'
    >
      <UserAvatar name={message?.user.name || session?.user.name} 
      ...
      <div className='flex-1'>
        ...
        <p className='text-primary/50 line-clamp-1'>
          {message?.translated?.["en"] || 'Get the conversation started...'}
        </p>
      </div>

      <div className='text-xs text-primary/50 text-right'>
        <p className='mb-auto'>
